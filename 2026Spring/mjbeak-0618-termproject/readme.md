...
**Workflow**
- **Input**:
  - [NnHispS CDS FASTA](https://www.uniprot.org/uniprotkb/A0A3G9K3K9/entry)
  - [AF-A0A3G9K3K9 PDB / PAE JSON](https://alphafold.ebi.ac.uk/entry/AF-A0A3G9K3K9-F1)
  - PKS25 UniProt Q54KU3 단백질 FASTA(uniprot.org/uniprot/Q54KU3)
  - [AF-Q54KU3 PDB / PAE JSON](https://alphafold.ebi.ac.uk/entry/AF-Q54KU3-F1)
- **Tool**:
...
**Workflow**
- **Input**:
  - 2.1에서 도출한 NnHispS.fasta / PKS25.fasta
...
NnHispS의 FASTA 서열을 입력으로 PKS/NRPS HMM 라이브러리를 대상으로 [pyhmmer](https://github.com/althonos/pyhmmer) (`hmmscan`)을 수행하였고, i-evalue < 1e-5를 기준으로 6개 도메인이 채택되었다(AMP-binding, ACP_1, KS, Docking domain, AT, ACP_2). 총 6개 도메인이 확인됨에 따라 5개의 도메인 간 gap이 정의되었으며, 이들을 잠재적 링커 후보로 간주하여 DSSP 기반 이차구조 조성을 분석하였다.
...
본 연구에서 구축한 분석 체계는 여러 단계를 거치는 순차적 과정으로 이루어져 있어, 대량의 PKS 분석에 수동으로 적용하기에는 현실적인 한계가 있다. 따라서 전체 과정을 자동화하더라도 개별적인 모듈(~result 4) 시행 결과를 재현할 수 있는지를 검증하고자 하였다. 아래는 result 4까지 각각 개별 모듈로 실행한 결과 얻은 링커 부위와 모든 모듈을 합친 자동화 코드로 얻은 링커 부위를 페어하게 비교한 것이다.
...
본 연구의 자동화 파이프라인은 분석 정확도를 유지하면서도 재현성과 확장성을 크게 향상시킬 수 있음을 확인하였다. 이는 개별 PKS 사례 분석을 넘어, 다양한 Type I PKS를 대상으로 한 고처리량(high-throughput) 링커 탐색 및 재설계 플랫폼으로 활용될 가능성을 보여준다. 어떤 type1 pks 서열을 주더라도 링커를 찾아주는 가장 간단한 workflow는 아래와 같다.
...
- **Output**:
  - [NnHispS_linker_report.csv](https://github.com/igchoi/IBT619-ProteinDesignTherory/upload/main/2026Spring/mjbeak-0618-termproject)
  - [PKS25_linker_report.csv](https://github.com/igchoi/IBT619-ProteinDesignTherory/upload/main/2026Spring/mjbeak-0618-termproject)
...
## supplementery 파일 (분석에 사용한 파일들은 링크로 연결됨)

| 파일 | 설명 |
|---|---|
| [`pks_linker_pipeline.py`](https://github.com/igchoi/IBT619-ProteinDesignTherory/blob/main/2026Spring/mjbeak-0618-termproject/pks_linker_pipeline.py) | Result V에서 검증한 자동화 스크립트 (Methods 2.2–2.5 
| [`QnA.md`](https://github.com/igchoi/IBT619-ProteinDesignTherory/blob/main/2026Spring/mjbeak-0618-termproject/QnA.md) | 방법론 정립 과정의 시행착오 전체 기록 |
