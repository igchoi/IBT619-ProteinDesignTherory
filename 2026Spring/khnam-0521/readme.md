...
`YSOh`: I have 2 questions. First, Binding affinity seems to be important in binder design. If a model is trained on affinity data, it could become a great tool that performs both scoring and prediction. Are there any parts in the seminar where affinity is mentioned? Second, Could RLA also be used as a 'reverse filter' to score the validity of large backbone ensembles generated by tools like RFdiffusion?
