# 분석에 사용한 코드 및 실행 명령 정리

이 문서는 항균 펩타이드 유사 후보 설계 및 Design B 추가 분석에 사용한 주요 코드와 실행 명령을 정리한 것이다. 전체 분석은 후보 서열 생성, 서열 기반 평가, ColabFold 구조 예측, 구조 평가, PyMOL 시각화, Design B 추가 분석 순서로 진행하였다.
